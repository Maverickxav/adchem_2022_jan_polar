module driver_acdc_J_3
use acdc_system_3, only : small_set_mode, n_charges               ! Simulation mode and numbers of charging states
use acdc_system_3, only : n_variables, variable_temp, variable_cs, variable_ipr   ! Information on parameters that vary based on input
use acdc_simulation_setup_3, only : solve_ss, j_flux              ! Logicals for the simulation type / obtaining J
use acdc_simulation_setup_3, only : sources_and_constants_3         ! For determining fitted concentrations
...
subroutine acdc_driver(neqn,nclust,nout_all,c,cs_ref,temperature,ipr,t_sim,t_tot,t_iter,ipar,ok,j_out)
    ...
    external feval_3, jeval_3, formation                    ! Subroutines for equations, jacobian and formation rate
    ...
    if (use_solver) then
        ...
        do i=1,nt_tot
            ...
            ok = .true.
            call DVODE (feval_3,neqn,c,t0,tfin,itol,rtol,atol,&   ! Calling the solver
                & itask,istate,iopt,work,lwork,iwork,liwork,&
                & jeval_3,mf,parameters,ipar)
            ...
            if (.not. ok) then                                  ! If there was a problem, try a shorter time interval
                ...
                tt(2) = tfin
                do j = 1, 2
                    call DVODE (feval_3,neqn,c,t0,tt(j),itol,rtol,atol,&    ! Calling the solver
                        & itask,istate,iopt,work,lwork,iwork,liwork,&
                        & jeval_3,mf,parameters,ipar)
        ...
        do while (t_iter_tot .lt. t00+t_max)
            ...
            ok = .true.
            call feval_3(neqn,ti,ci,f_out,parameters,ipar)
            ...
            call feval_3(neqn,ti+tt(1)/2.d0,c,f_out_mp,parameters,ipar) ! f at midpoint
            do n = 1,neqn
...
subroutine fix_fitted_c(neqn,nclust,c,ci,ipar)
    ...
    if (firstcall) then
        ...
        call sources_and_constants_3(nclust,source,isconst,fitted) ! Get the fitting settings (variable "fitted")
    end if
    ...
    do n = 1, fitted(1,0)    ! Loop over the clusters whose concentrations are fitted (e.g. sum([1AnD])=const.)
        ...
        if (ipar(1) .eq. 0) then
            c_fit = ci(k)    ! If the settings have changed, the sum([1AnD])=c_fit has been fed in as [1A] in the driver call;
                             ! feval_3 will change ipar(1) to 1 when the integration begins...
