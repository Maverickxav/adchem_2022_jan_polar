...
! use clusterin_1, only : cluster_dynamics_1
! use acdc_simulation_setup_1, only : get_system_size_1
! use clusterin_2, only : cluster_dynamics_2
! use acdc_simulation_setup_2, only : get_system_size_2
    ...
    real(kind(1.d0)) :: j_acdc_N, j_acdc_D, j_acdc_I,  j_acdc_M, j_acdc_total                                      ! Simulated total formation rate (m^-3 s^-1)
    ...
    integer, parameter :: n_spec_example = 7
    ...
    output_dir='/Users/carlton/Work/ACDC_versions/Clusterin_multiple_chemistries/ClusterIn/ClusterIn_msa/output'
    ...
    names_vapor_all(3)(:) = 'N'
    names_vapor_all(4)(:) = 'D'
    names_vapor_all(5)(:) = 'Ii'
    names_vapor_all(6)(:) = 'Io'
    names_vapor_all(7)(:) = 'Ms'
   
    c_vapor_all = (/1.d10, 2.d6, 1.d9, 1.d6, 1.d5, 1.d6, 1.d6/)*1.d6
...
!-----------------------------------------------------------------------------------
    ...
    OPEN(10010,FILE=trim(adjustl(output_dir))//'/Jnucl_N.dat',STATUS='REPLACE',ACTION='WRITE')
    OPEN(10011,FILE=trim(adjustl(output_dir))//'/Jnucl_D.dat',STATUS='REPLACE',ACTION='WRITE')
    OPEN(10012,FILE=trim(adjustl(output_dir))//'/Jnucl_I.dat',STATUS='REPLACE',ACTION='WRITE')
    OPEN(10013,FILE=trim(adjustl(output_dir))//'/Jnucl_M.dat',STATUS='REPLACE',ACTION='WRITE')
    OPEN(10014,FILE=trim(adjustl(output_dir))//'/Jnucl_total.dat',STATUS='REPLACE',ACTION='WRITE')
    l_dynamic = .true.
        ...
        if (trim(coag_test_name) .eq. 'coag_molec') then
            ...
            do k = 1,n_clustering_syst
                ...
                if (k .eq. 1) then
                    ...
                    ! write(*,*) 'L261 nclust_syst(k)', nclust_syst(k)
                    ! stop
                else if (k .eq. 2) then
                    call get_system_size_2(nclust_syst=nclust_syst(k))
                else if (k .eq. 3) then
                    call get_system_size_3(nclust_syst=nclust_syst(k))
                else if (k .eq. 4) then
                    call get_system_size_4(nclust_syst=nclust_syst(k))
                end if
            ...
            write(*,*) 'L272 nclust_syst_max',nclust_syst_max
            ! stop
...
!-----------------------------------------------------------------------------------
    ...
    do it = 1,1000
        ...
        if (l_dynamic) then
            ...
            nmols_evap_all = (/2.d0, 5.d0, 5.d0, 1.d0,  2.d0, 2.d0,2d0/)
            ...
            if (l_dynamic) then
                ...
                if (trim(coag_test_name) .eq. 'coag_molec') then
                    ...
                    if (k .eq. 1) then
                        ...
                        write(*,*) names_vapor_syst(1:ind_v,k)
                        call cluster_dynamics_3(names_vapor_syst(1:ind_v,k),c_vapor_tmp(1:ind_v),cs_ref,temp,ipr,time_res,0.d0,&
                        &    j_acdc,diameter_acdc,c_out=c_out,&
                        &    naero=naero,dp_aero_lim=dp_aero_lim,dp_aero=dp_aero,c_aero=c_aero,&
                        &    c_evap=c_evap_tmp,nmols_evap=nmols_evap_tmp(1:ind_v),&
                        &    c_coag_molec=c_coag_aero_tmp(:,1:ind_v),&
                        ...
                        write(*,*) names_vapor_syst(1:ind_v,k)
                        write(*,*) c_vapor_tmp(1:ind_v)/1d6
                        
                        call cluster_dynamics_4(names_vapor_syst(1:ind_v,k),c_vapor_tmp(1:ind_v),cs_ref,temp,ipr,time_res,0.d0,&
                        &    j_acdc,diameter_acdc,c_out=c_out,&
                        &    naero=naero,dp_aero_lim=dp_aero_lim,dp_aero=dp_aero,c_aero=c_aero,&
                        &    c_evap=c_evap_tmp,nmols_evap=nmols_evap_tmp(1:ind_v),&
                        &    c_coag_molec=c_coag_aero_tmp(:,1:ind_v),&
                ...
                ! write(*,*) 'clust_molec', '', sum(clust_molec_tmp)
                write(*,*)
        ...
        write(10010,*) j_acdc_N*1d-6
        write(10011,*) j_acdc_D*1d-6
        write(10012,*) j_acdc_I*1d-6
        write(10013,*) j_acdc_M*1d-6 
        write(10014,*) (j_acdc_N + j_acdc_D + j_acdc_I + j_acdc_M )*1d-6
        t = t + time_res
