module acdc_simulation_setup_1
use acdc_system_1
...
subroutine sources_and_constants_1(neqn,source,isconst,fitted)
    ...
    if (solve_ss) then
        ...
        if (small_set_mode) then
            ...
            call monomer_indices_1(n_monomers)
...
end subroutine sources_and_constants_1
...
subroutine get_system_size_1(neq_syst,nclust_syst,nout_syst,nclust_out,nrange_coag,nrange_out,diameter_max_syst)
...
end subroutine get_system_size_1
...
subroutine get_vapor_indices(names_vapor,n1vapor,nmol_vapor)
    ...
    call monomer_names_1(names_monomers)
    call monomer_indices_1(n_monomers)
    call molecule_names_1(names_mol_types)
...
subroutine get_bound_index(nmol_vapor,molec_vapor,nclust_bound,diff_molec_vapor)
    ...
    if (firstcall) then
        ...
        call get_bound_1(bound_clusters,nmols_bound)
        ...
        call get_charging_state_1(ch_state)
    ...
    if (any(abs(frac_bound(ind(1),:)-frac_mol_types) .gt. diff_th)) then
        ...
        ! write(*,*) 'Evaporating particles'' molar composition in chem 1 ', frac_mol_types, nmol_vapor
        ! write(*,*) 'Closest boundary composition ', frac_bound(ind(1),:)
        ! write(*,*) 'nclust_bound ', nclust_bound
    end if
...
subroutine get_scav_parameters_1(names_vapor,temperature,c_coag,cs_per_clust,&
    ...
    if (firstcall) then
        ...
        call get_system_size_1(nclust_syst=nclust_syst)
        ...
        call get_diameter_1(dp_clust)
        call get_mass_1(mp_clust)
        ...
        call molecule_names_nocharge_1(names_nocharge_acdc)
        call get_nmols_nocharge_1(nmols_nocharge_clust_acdc)
    ...
    if (.not. present(temperature)) then
        write(*,*) 'get_scav_parameters_1: Temperature must be given as input at every call in the current implementation'
        stop
    end if
...
end subroutine get_scav_parameters_1
...
subroutine get_jbin_parameters(c_out_clust,c_per_aero,comp_nocharge_per_aero,nmols_nocharge_out,&
    ...
    if (firstcall) then
        ...
        call get_system_size_1(nclust_out=nclust_out)
        ...
        call get_diameter_out_1(dp_clust_out)
        dp_clust_out = dp_clust_out*1.d-9           ! nm -> m
        
        ! Get cluster compositions in terms of summed neutral and charged molecules of each species
        call molecule_names_nocharge_1(names_nocharge_acdc)
        call get_nmols_nocharge_out_1(nmols_nocharge_out_acdc)
        names_nocharge_saved = names_nocharge_acdc
...
end module acdc_simulation_setup_1
