module driverAMsD
use acdc_system_AMsD, only : nclust, neqn => neq				! numbers of clusters and equations
use acdc_system_AMsD, only : nout_neu						! numbers for formation fluxes
!use acdc_system, only : nout_neg, nout_pos
use solution_settings
use monomer_settingsAMsD , only : sources_and_constantsD		! for determining fitted concentrations

implicit none

contains

subroutine acdc_driverAMsD(c,cs_ref,temperature,t_max,t_iter,solve_ss,use_solver,ipar,ok,j_out)
...
!	real(kind(1.d0)), intent(in) :: q_ion				! ion source rate (1/s/m^3)
	...
	if (solve_ss) then
		...
		call formationAMsD(neqn,c,j_tot,j_by_charge,j_by_cluster,j_all,ipar,parameters)
		j_out = (/j_tot/)
...
subroutine fix_fitted_c(c,ci,ipar)
	...
	if (firstcall) then
		firstcall = .false.
		call sources_and_constantsD(source,isconst,fitted,ipar) ! get the fitting settings (variable "fitted")
	end if
