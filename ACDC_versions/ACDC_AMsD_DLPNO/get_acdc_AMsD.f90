...
subroutine get_acdc_AMsD(c_acid,c_base,c_org,cs_ref,temp,time,solve_ss,j_acdc,diameter_acdc,c)
use acdc_system_AMsD, only : nclust, neq					! number of clusters and equations
use acdc_systemAMsD, only : cluster_names				! names of the clusters and fluxes
use acdc_systemAMsD, only : n1A							! cluster numbers of acid and base monomers
use acdc_systemAMsD, only : nout_neu					! numbers for formation fluxes
use acdc_systemAMsD, only : nmols_out_neutral, nmolA	! criteria for "nucleation"
use acdc_systemAMsD, only : diameter_max				! max. mass diameter (nm)
use driverAMsD, only : acdc_driver						! driver
...
!	real(real_x), intent(in) :: q_ion				! ion source rate (1/s/m^3)
	...
	if (firstcall) then
		...
		do n = 1, nclust
			...
			else if (trim(c_names(n)(:)) .eq. '1Ms') then
				n1org = n
			end if
		end do
