...
! Don't update IO3- concentrations after multiphase chemistry module (KPP_Proceed)
   ...
   ! USE multilayer
    ...
    REAL(dp) :: start_write_time,finish_write_time
    REAL(dp) :: start_write_reg_time,finish_write_reg_time
    ...
    integer :: increment  !!! number of outputs
    integer :: ncids(3)
    integer, dimension(nr_outputs):: general_ids
...
!    t_end = 60.
	...
	EmissionNH3_birds=EmissionNH3_birds*1E-4*1E3/17D0*Na*3600D0
...
! Discrete wave lengths of solar irradiance
 ...
 END DO
    ...
    ! species_names=SPC_NAMES
    call open_write_nc_files(.True.,.false.,.false., ncids, general_ids, species_names=SPC_NAMES)
    write(*,*) 'l1575', ncids(1), general_ids(1)
    ...
    loop: DO WHILE ((time) <= t_end )!t_end)
...
END If
IF (AMsD) then
  DO i=1,chem_4%neq_syst !31
         conc_diff=c_clusters_m(:,i)    
         CALL diffusion1D_variable_z(Kz,conc_diff,k_uper_BC,dt)
         c_clusters_m(:,i)=conc_diff
  END DO
end if	
...
! *******************************************************************************************************************************
	...
	EmissionI2=O3_ppb*(Iion_aq**1.3)*(1.74D9-6.54D8*LOG(windspeed(tr)))*1D-4*1D-9*Na/(24D0) !  molec cm^-2 s^-1  #molec cm^-2 h^-1
    ...
    DO j=1, nz
        ...
        if (clust_firstcall) then
            ...
            if (AMsD) then
                ...
                do kk=1, chem_4%nclust_out
                    volume_chem4 = sum(real(chem_4%clust_out_molec(kk,:),KIND=dp)/Na*Mx_chem4(:)/qX_chem4(:))
                    DO ii=2,nr_bins
                        IF (volume_chem4 .LT. vp(ii)) THEN
                            chem_4%ind_out_bin(kk)=ii-1
                            EXIT
                        END IF
                    END DO
                    IF (chem_4%ind_out_bin(kk) .EQ. 0) THEN
            ...
            if (l_coagulation_loss) then
                ...
                if (AMsD) then
                    DO i=1,chem_4%nclust_syst
                        ...
                        chem_4%c_p_clust(11,i) = REAL(chem_4%clust_molec(i,3),KIND=dp)
        ...
        if (l_coagulation_loss) then
            ...
            ! write(*,*) 'systems:',systems+n_3comp_sys
            CALL coagulation_clusterin(N_bins1,V_bins1,c_p1,c_p_backg, d_p1,dp_dry1,dt,dens_p1,MX,qX,chem_1,chem_2,chem_3,chem_4,&
            systems+n_3comp_sys,three_comp,log_array, ncases)
        ...
        IF (AN)then
            ...
            DO ii=1,chem_1%nclust_out
                ...
                c_p(j,1,i) = c_p(j,1,i)+real(chem_1%clust_out_molec(ii,1),KIND=dp)*chem_1%conc_out_all(ii)*1D-6 ! to #/cm3
        ...
        if (AMsD)then
            ...
            DO ii=1,chem_4%nclust_out
                ...
                i = chem_4%ind_out_bin(ii)
                
                N_bins(j,i) = N_bins(j,i)+chem_4%conc_out_all(ii)
                ! write(*,*) 
                c_p(j,1,i) = c_p(j,1,i)+real(chem_4%clust_out_molec(ii,1),KIND=dp)*chem_4%conc_out_all(ii)*1D-6
                c_p(j,9,i) = c_p(j,9,i)+real(chem_4%clust_out_molec(ii,2),KIND=dp)*chem_4%conc_out_all(ii)*1D-6
                c_p(j,11,i) = c_p(j,11,i)+real(chem_4%clust_out_molec(ii,3),KIND=dp)*chem_4%conc_out_all(ii)*1D-6
...
!	  END IF
	...
	Hion=10**(-pH_bulk1)
     ...
     !fHCl=conc1(ind_HClaq)/(conc1(ind_Clion)+conc1(ind_HClaq)+1D-100)
	  ...
	  conc1(ind_NH3aq) = conc_old(ind_NH3aq)/(conc_old(ind_NH4ion)+conc_old(ind_NH3aq)+1D-100)*SUM(c_p(j,4,:)) ! Total N(III) particle conc
    ...
    !   write(*,*) 'L3052'  , '' , conc1(ind_IO3ion), '', conc1(ind_HIO3aq) 
     conc_old=conc1
...
END DO  
    ...
    !     CALL cpu_time(start_write_reg_time)
        
    !     CALL write_output(time,conc,index_cond,N_bins,V_bins,d_p,dp_dry,c_p,v_dep,v_wet1,dens_p,&
    !     index_st,mcm_prop(1,:)*1D-3,psat_org,E_gases,Kz,vd_gas,tr,&
    !     PN_marine,Jnucl_N, Jnucl_D,CS_H2SO4,RH,Ts(:,tr),pH)

    !     CALL cpu_time(finish_write_reg_time)
    
    !     PRINT '("Simulation time for regualr write = ",f6.3," hours")', (finish_write_time-start_write_time)/3600.
    ! END IF


    
    ! if (tr>((days-days_upwind_save)*(24*60)-1)) then
    if (tr>5) then
        ...
        PRINT '( "Simulation time for netcdf = ",f6.3," hours")', (finish_write_time-start_write_time)/3600.
    END IF    
        ! increment=increment+1

time = time + dt
!    tr = tr + 1
! if (int(time)==120) then
...
tr = CEILING(time/60D0)
   ...
   !write(*,*) time
    ...
    write(*,*) 'End loop Time: ', ' ', tr, ' ', int(time )
    END DO loop
    ...
    !!!!! close netcdf files
    call open_write_nc_files(.False.,.false.,.true.,ncids, general_ids)
