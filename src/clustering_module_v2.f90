...
subroutine allocate_chem_dimensions(chem_v, MX_v,qX_v, c_clusters,c_clusters_v, nz, n_clustering_vapors, kk)
    ...
    elseif (kk.eq.5) then
        call get_system_size_5(neq_syst=chem_v%neq_syst)
    end if
...
subroutine initialize_chem(chem_v, kk, n_clustering_vapors)
    ...
    elseif (kk .eq. 5) then
        write(*,*) 'calling system 4'
        call get_system_size_5(neq_syst=chem_v%neq_syst,nclust_syst=chem_v%nclust_syst,nclust_out=chem_v%nclust_out)
    end if
...
Subroutine clustering_subroutine(chem_1, chem_2,chem_3, chem_4, chem_5, clust_firstcall, n_clustering_vapors,n_clustering_vapors_3comp, nr_bins,&
     cH2SO4, cDMA, cNH3,cHIO2, cHIO3, cMSA,&
     Jnucl_N_out, Jnucl_D_out, Jnucl_I_out,Jnucl_Ms_out, Jnucl_AND_out, &
     dia_n, dia_dma,dia_hio,dia_msa, dia_and, CS_H2SO4, &
     T, press, ipr,dt, d, d_p, m_p, N_bins_in, Mx, qX,n_evap, comp_evap, &
     clustering_systems, c_clusters1, c_clusters2,c_clusters3, c_clusters4, c_clusters5, layer,&
      l_cond_evap, l_coag_loss, three_comp_sys, two_comp, AN, AD, IiIo, AMsD,AND )
   
    IMPLICIT NONE

    type(clustering_mod), intent(inout),optional ::chem_1, chem_2,chem_3, chem_4, chem_5
    ...
    REAL(DP), intent(out), optional :: Jnucl_N_out, Jnucl_D_out, Jnucl_I_out, Jnucl_Ms_out, Jnucl_AND_out, dia_n, dia_dma, dia_hio, dia_msa, dia_and
    ...
    REAL(dp), intent(inout), optional ::  c_clusters1(:),c_clusters2(:),c_clusters3(:),c_clusters4(:),c_clusters5(:)
    
    ! real(dp), optional :: 
    
    integer:: i, kk    
    logical, intent(in):: l_cond_evap,l_coag_loss, three_comp_sys, two_comp,AN, AD, IiIo, AMsD, AND
    ...
    if (clust_firstcall) then
        ...
        if (AND) then     
            CALL initialize_chem(chem_5,5, n_clustering_vapors_3comp)
            write(*,*) 'nceq_syst', chem_5%neq_syst, '', chem_5%nclust_syst,'',chem_5%nclust_out
        end if    
    ...
    !!! update gas concentrations by accounting for the the evaporated gas species from the aerosol module
    ...
    if (n_evap > 0D0) then !!! get the composition of evaportating particles
        if (AN) then
        ...
        if(AND) then    
            chem_5%nmols_evap(1) = comp_evap(1)*1.D6/n_evap
            chem_5%nmols_evap(2) = comp_evap(4)*1.D6/n_evap ! MSA
            chem_5%nmols_evap(3) = comp_evap(11)*1.D6/n_evap ! DMA
        end if
    end if    
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (AND) then
        ...
        chem_5%conc_vapor=(/C_ACID ,C_BASE, c_DMA/)
        
        if (l_cond_evap) then
        
            CALL cluster_dynamics_5(chem_5%names_vapor,chem_5%conc_vapor,CS_H2SO4,T,ipr,dt,0.d0,&
            &    Jnucl_AND_out,dia_and,c_inout=c_clusters5,naero=nr_bins,dp_aero_lim=d ,mp_aero=m_p,c_aero=N_bins_in,dp_aero=d_p,pres=Press, &
            c_evap=n_evap,nmols_evap=chem_5%nmols_evap,&
            c_coag_molec=chem_5%conc_coag_molec,c_coag_clust=chem_5%conc_coag_clust,clust_molec=chem_5%clust_molec, &
            c_out_bin=chem_5%conc_out_bin,comp_out_bin=chem_5%comp_out_bin,&
            c_out_all=chem_5%conc_out_all,clust_out_molec=chem_5%clust_out_molec)

        elseif (l_coag_loss) then
            
            CALL cluster_dynamics_5(chem_5%names_vapor,chem_5%conc_vapor,CS_H2SO4,T,ipr,dt,0.d0,&
            &    Jnucl_AND_out,dia_and,c_inout=c_clusters5, naero=nr_bins,dp_aero_lim=d ,dp_aero=d_p,mp_aero=m_p,c_aero=N_bins_in,pres=Press,&
            c_evap=n_evap,nmols_evap=chem_5%nmols_evap,&
            c_coag_clust=chem_5%conc_coag_clust,clust_molec=chem_5%clust_molec,&
            c_out_bin=chem_5%conc_out_bin,comp_out_bin=chem_5%comp_out_bin,&
            c_out_all=chem_5%conc_out_all,clust_out_molec=chem_5%clust_out_molec) 
        ...
        cH2SO4=chem_5%conc_vapor(1)*1d-6
        cNH3=chem_5%conc_vapor(2)*1d-6
        cDMA=chem_5%conc_vapor(3)*1d-6
