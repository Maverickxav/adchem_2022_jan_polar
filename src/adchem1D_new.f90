...
! Don't update IO3- concentrations after multiphase chemistry module (KPP_Proceed)
   ...
   ! USE multilayer
    ...
    REAL(dp), DIMENSION(Nz) :: Jnucl,JnuclDMA,Jnucl_org, Jnucl_SAorg ! New particle formation rate # cm^-3 s^-1
    ...
    CHARACTER*16                  :: EM_VAR(22)                        ! Name of the 22 VOC's
	 ...
	 ! ACDC parameters:
    REAL(dp)                      :: c_acid,c_base,c_org,diameter_acdc,diameter_acdcDMA
	...
	REAL(dp)                      :: X_NH3,F_NH3ocean,kg_NH3,cNHx_ocean,Sc_NH3,C_D, &
	                                 H_NH3_sea,S_sea,pKa_NH3_sea,pH_ocean,H_prim_NH3_sea,&
...
!    t_end = 60.
    ...
    OPEN(231,FILE='output/Jnucl_st_'//sim//date,STATUS='REPLACE',ACTION='WRITE')
...
! Mixing of ACDC clusters, added by Pontus 2021-05-26
  DO i=1,63
         conc_diff=c_clusters(:,i)    
         CALL diffusion1D_variable_z(Kz,conc_diff,k_uper_BC,dt)
         c_clusters(:,i)=conc_diff
  END DO
  DO i=1,31
         conc_diff=c_clustersDMA(:,i)    
         CALL diffusion1D_variable_z(Kz,conc_diff,k_uper_BC,dt)
         c_clustersDMA(:,i)=conc_diff
  END DO
...
H_prim_DMA_sea = H_DMA_sea/(1D0+10**(-pH_ocean+pKa_NH3_sea)) ! Use the same pKa dependance as for ammonia (according to carpenter)
...
! New particle formation
 ...
 CALL  get_acdc_J(c_acid,c_base,c_org,CS_H2SO4(j),Ts(j,tr),ipr,dt,solve_ss,Jnucl(j),diameter_acdc,Nuc_by_charge,c_clusters1)
...
! Update cluster and vapour concentrations!
 ...
 CALL  get_acdc_D(c_acid,c_base,c_org,CS_H2SO4(j),Ts(j,tr),dt,solve_ss,JnuclDMA(j),diameter_acdcDMA,c_clusters2)
 N_bins(j,1) = N_bins(j,1)+Jnucl(j)*dt
 N_bins(j,2) = N_bins(j,2)+JnuclDMA(j)*dt
 c_p(j,:,1) = c_p(j,:,1)+c_p_nucl*Jnucl(j)*1D-6*dt
 c_p(j,:,2) = c_p(j,:,2)+c_p_nuclDMA*JnuclDMA(j)*1D-6*dt
 ! Update cluster and vapour concentrations!
 c_clusters(j,:)=c_clusters1
 ...
 END DO        
 write(*,*) 'l2015 Nconc_evap', sum(N_bins(:,1),dim=1), sum(N_bins(:,2),dim=1) , sum( sum(c_p(:,:,1), dim=1),dim=1), sum( sum(c_p(:,:,2), dim=1),dim=1)
 END IF
...
END DO  
    ...
    psat_org(756:761)=1D-100 ! RO2
     ...
     Dorg=1D-7*Ts(j,tr)**1.75D0*SQRT(1/(Mair*1D3)+1/mcm_prop(1,:))/&
		  ...
		  CALL condensation(N_bins1,V_bins1,Ps(j,tr),Ts(j,tr),RH(j),&
			   ...
			   yorg,Dorg,CS_H2SO4(j),CS_air(j),c_p_backg,dt)
    ...
    CALL write_output(time,conc,index_cond,N_bins,V_bins,d_p,dp_dry,c_p,v_dep,v_wet1,dens_p,&
    index_st,mcm_prop(1,:)*1D-3,psat_org,E_gases,Kz,vd_gas,tr,&
    PN_marine,Jnucl,CS_H2SO4,RH,Ts(:,tr),pH)
    END IF
