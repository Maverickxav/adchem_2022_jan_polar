...
! Don't update IO3- concentrations after multiphase chemistry module (KPP_Proceed)
   ...
   ! USE multilayer
    ...
    REAL(dp), DIMENSION(Nz) :: Jnucl_N,Jnucl_D,Jnucl_org, Jnucl_SAorg ! New particle formation rate # cm^-3 s^-1
    ...
    CHARACTER*16                  :: EM_VAR(22)                        ! Name of the 22 VOC's
	 ...
	 ! ACDC parameters:
    REAL(dp)                      :: c_acid,c_base,c_org,diameter_acdc,diameter_acdcDMA,cDMA
    ...
    REAL(dp), allocatable     :: c_clusters_n(:,:), c_clusters_d(:,:), c_clusters_m(:,:),c_clusters1(:), c_clusters2(:),c_clusters3(:)
    ...
    real(dp) ::  real_tmp1, real_tmp2,  volume_chem1,volume_chem2
    ...
    ! allocate(chem_3%names_vapor(n_clustering_vapors_3comp))
    ! allocate(chem_3%conc_vapor(n_clustering_vapors_3comp))
    ! allocate(chem_3%nmols_evap(n_clustering_vapors_3comp))
   
    ! allocate(chem_3%nconc_evap(nz))
    ! allocate(names_vapor_syst_readin_3comp(n_clustering_vapors_3comp,1))
    
    allocate(names_vapor_syst_readin(n_clustering_vapors,n_clustering_vapors))
    ! allocate(volume_chem1(n_clustering_vapors))
    ! allocate(volume_chem2(n_clustering_vapors))
    allocate(Mx_chem1(n_clustering_vapors))
    allocate(Mx_chem2(n_clustering_vapors))
    allocate(Mx_chem3(n_clustering_vapors_3comp))
    
    allocate(qX_chem1(n_clustering_vapors))
    allocate(qX_chem2(n_clustering_vapors))
    allocate(qX_chem3(n_clustering_vapors_3comp))
    ...
    allocate(c_clusters_m(nz,chem_3%neq_syst))
    
    allocate(c_clusters1(chem_1%neq_syst))
    allocate(c_clusters2(chem_2%neq_syst))
    ! allocate(c_clusters3(chem_3%neq_syst))
    
    c_clusters1=0d0; c_clusters2=0d0;!c_clusters3=0d0;
    c_clusters_n=0d0;c_clusters_n=0d0;! c_clusters_m=0d0;
...
!    t_end = 60.
    ...
    do kk=1, clustering_systems
        if (kk<=2) then
            WRITE (filename, fmt='(a,I1a)') './ACDC_versions/Clusterin_multiple_chem_3comp/cluster_chem_spec_',kk,'.txt'
            ...
            !     do while (i .lt. n_vapor_syst)
            !         i = i+1
            !         read (400,*) names_vapor_syst_readin_3comp(i,1)
            !     end do
            !     close(400)
            !     write(*,*) 'l648',' ', names_vapor_syst_readin_3comp(:,1)
            end if      
     end do
     
    chem_1%names_vapor=names_vapor_syst_readin(:,1)
    chem_2%names_vapor=names_vapor_syst_readin(:,2)
    ! chem_3%names_vapor=names_vapor_syst_readin_3comp(:,1)
    
    
    
     write(*,*) 'L572', ' ', chem_1%names_vapor
     write(*,*) 'L573', ' ', chem_2%names_vapor
    !  write(*,*) 'L573', ' ', chem_3%names_vapor
...
IF(nucleation_index==1) THEN
    ...
    DO j=1, nz
        ...
        if (clust_firstcall) then
                ...
                write(*,*) 'sum(chem_1%clust_molec)',sum(chem_1%v_clust),sum(chem_2%v_clust)
        ...
        if (l_coagulation_loss) then
            ...
            CALL coagulation_clusterin(N_bins1,V_bins1,c_p1,c_p_backg, d_p1,dp_dry1,dt,dens_p1,MX,qX,chem_1,chem_2,n_clustering_vapors)
