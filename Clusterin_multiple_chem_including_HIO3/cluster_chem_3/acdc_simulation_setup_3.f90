module acdc_simulation_setup_3
use acdc_system_3
...
subroutine sources_and_constants_3(neqn,source,isconst,fitted)

    implicit none
    
    real(kind(1.d0)) :: source(neqn)
    logical :: isconst(neqn)
    integer :: neqn, fitted(neqn,0:neqn), cluster_numbers(n_1Ii_clusters), n, i
    ...
    if (solve_ss) then
        ...
        if (small_set_mode) then
            ...
            call monomer_indices_3(n_monomers)
...
end subroutine sources_and_constants_3
...
subroutine get_system_size_3(neq_syst,nclust_syst,nout_syst,nclust_out,nrange_coag,nrange_out,diameter_max_syst)
...
end subroutine get_system_size_3
...
subroutine get_vapor_indices(names_vapor,n1vapor,nmol_vapor)
    ...
    call monomer_names_3(names_monomers)
    call monomer_indices_3(n_monomers)
    call molecule_names_3(names_mol_types)
...
subroutine get_bound_index(nmol_vapor,molec_vapor,nclust_bound,diff_molec_vapor)
    ...
    if (firstcall) then
        ...
        call get_bound_3(bound_clusters,nmols_bound)
        ...
        call get_charging_state_3(ch_state)
...
subroutine get_scav_parameters_3(names_vapor,temperature,c_coag,cs_per_clust,&
    ...
    integer, intent(out), optional :: nmols_nocharge_clust(:,:)
    ...
    if (firstcall) then
        ...
        call get_system_size_3(nclust_syst=nclust_syst)
        ! write(*,*) nclust_syst
        ...
        call get_diameter_3(dp_clust)
        call get_mass_3(mp_clust)
        ...
        call molecule_names_nocharge_3(names_nocharge_acdc)
        call get_nmols_nocharge_3(nmols_nocharge_clust_acdc)
    ...
    ! write(*,*) nmols_nocharge_clust_saved
    
    if (present(names_nocharge)) names_nocharge = names_nocharge_saved    
    ! nmols_nocharge_clust = nmols_nocharge_clust_saved
    ...
    if (.not. present(temperature)) then
        write(*,*) 'get_scav_parameters_3: Temperature must be given as input at every call in the current implementation'
        stop
    end if
...
end subroutine get_scav_parameters_3
...
subroutine get_jbin_parameters(c_out_clust,c_per_aero,comp_nocharge_per_aero,nmols_nocharge_out,&
    ...
    if (firstcall) then
        ...
        call get_system_size_3(nclust_out=nclust_out)
        ...
        call get_diameter_out_3(dp_clust_out)
        dp_clust_out = dp_clust_out*1.d-9           ! nm -> m
        
        ! Get cluster compositions in terms of summed neutral and charged molecules of each species
        call molecule_names_nocharge_3(names_nocharge_acdc)
        call get_nmols_nocharge_out_3(nmols_nocharge_out_acdc)
        names_nocharge_saved = names_nocharge_acdc
        ...
        do i = 1, nclust_out
            ...
            do j = 1, naero
                if ((dp_clust_out(i) .ge. dp_aero_lim(j)) .and. (dp_clust_out(i) .lt. dp_aero_lim(j+1))) then
                    ...
                    aero_for_clust_out(i) = 2
                    write(*,*) 'Outgrown cluster of diameter ', dp_clust_out(i)*1.d9, ' nm not found, so place in placed in aerosol bin no. 1 '
                    lfound = .true.
                    exit
...
end module acdc_simulation_setup_3
