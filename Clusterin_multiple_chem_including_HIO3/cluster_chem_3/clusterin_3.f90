...
subroutine cluster_dynamics_3(names_vapor,c_vapor,cs_ref,temp,ipr,t_sim,t_tot,j_total,diameter_approx,&
&    c_inout,&
...
use acdc_simulation_setup_3, only : solve_ss, sources_and_constants_3                           ! Logicals for the solution approaches
use acdc_system_3, only : small_set_mode, n_neutral_monomers, n_charges, nclust_max           ! Simulation mode and numbers of vapors and charging states
use acdc_simulation_setup_3, only : get_system_size_3, get_vapor_indices                        ! Parameters related to the simulation system size

use acdc_simulation_setup_3, only : get_bound_index, get_scav_parameters_3, get_jbin_parameters, get_ambient_params
use acdc_simulation_setup_3, only : group_size_bins, nbins

use driver_acdc_J_3, only : acdc_driver, use_solver                                           ! Driver for the numerical integration
...
!--------------------------------------------------------------------------------------------------
    ...
    if (firstcall) then
        ...
        call get_system_size_3(neq_syst=neq_syst,nclust_syst=nclust_syst,nout_syst=nout_syst,diameter_max_syst=diameter_max_syst)
        ...
        call sources_and_constants_3(neq_syst,source,isconst,fitted)
...
!--------------------------------------------------------------------------------------------------
    ...
    if (present(c_coag_molec) .or. present(c_coag_clust)) then
        ...
        if (.not. set_c_coag) then
            ...
            call get_system_size_3(nrange_coag=nrange_coag)
            ...
            write(*,*) '         - Vapor concentrations must have been set to vary freely in sources_and_constants_3'
    ...
    if (present(c_out_bin) .or. present(c_out_all)) then
        ...
        if (.not. set_jbins) then
            ...
            call get_system_size_3(nclust_out=nclust_out,nrange_out=nrange_out)
    ...
    if (present(c_evap)) then
        if (c_evap .gt. 0.d0) then
            ...
            if (present(nmols_evap)) then
                ...
                write(*,*) 'If c_evap is used, also nmols_evap must be given to cluster_dynamics_3'
                stop
            end if
...
!--------------------------------------------------------------------------------------------------
    ...
    if (present(c_coag_molec) .or. present(c_coag_clust)) then
        ...
        if (present(c_coag_molec)) then
            ...
            call get_scav_parameters_3(names_vapor=names_vapor,temperature=temp,c_coag=c_coag,&
...
end subroutine cluster_dynamics_3

end module clusterin_3
