...
!~~~> Time loop
  ...
  SUBROUTINE KPP_Proceed(CONS,TSTARTp,TENDp,TEMPp,RHp,PRESSp,O2p,N2p,Mp,H2Op,RES1p,RES2p,CSp,cwp,H_ionp,OH_ionp,Jp,KVALUESp)
	...
	REAL(kind=dp), INTENT(IN), DIMENSION(50) :: CSp
    REAL(kind=dp), INTENT(IN) :: Jp(NPHOT),KVALUESp(NKVALUES)
	REAL(kind=dp), DIMENSION(50) :: kmt_molec,H_molec,k_molec,Hprim
...
H_molec(5)= 1.1D0*EXP(2020D0*(1D0/TEMP-1D0/298D0)) !HCl
...
H_molec(25)= 4.964D6*1D-3*1.01235D5 ! COSMOtherm 2.1D5*EXP(8700D0*(1D0/TEMP-1D0/298D0)) !HIO3
...
H_molec(34)= 1D7*EXP(2580D0*(1D0/TEMP-1D0/298D0)) !DMSO, Hoffmann et al., 2016 ! 9.4D2*101.325*EXP(1300D0*(1D0/TEMP-1D0/298D0))! Watts and Brimblecombe (1987)
H_molec(35)= 1D7*EXP(5390D0*(1D0/TEMP-1D0/298D0)) !DMSO2, Hoffmann et al., 2016 ! 4.9D2 De Bruyn et al (1994) 
H_molec(36)= 1.68D9*EXP(1760D0*(1D0/TEMP-1D0/298D0)) !MSIA, COSMO-therm & Hoffmann et al., 2016
H_molec(37)= 1.33D7 ! HPMTF COSMOtherm (non-hydrated form)
...
H_molec(47)= 1.33D7 ! HPMTF COSMOtherm (non-hydrated form) assumed the same for HOOCH2SCO
H_molec(48)=2.1D5*EXP(8700.0*(1D0/TEMP-1D0/298.15)) ! ! mol/(kg atm) HNO3(g)<->HNO3(aq)
H_molec(49)=1D0/(98D-3*exp(-11.695D0+10156D0*(1D0/360.15D0-1D0/TEMP+0.38D0/545D0*(1D0+log(360.15/TEMP)-360.15/TEMP)))) !  Kulmala and Laaksonen (1990) (atm) modified by Noppel et al., 2002  ! mol/(kg atm) H2SO4(g)<->H2SO4(aq)
H_molec(50)= 1.13D11*EXP(1760D0*(1D0/TEMP-1D0/298D0)) ! mol/(kg atm) Henry's law coeff for CH3SO2H(g)<->CH3SO2H(aq), COSMOtherm! MSA COSMOtherm 
   ...
   kClO_aq_g=kmt_molec(3)/Hprim(3)
   kClO2_g_aq=kmt_molec(4) 
   ...
   kHOOCH2SCO_g_aq=kmt_molec(47)
   kHOOCH2SCO_aq_g=kmt_molec(47)/Hprim(47)
   kHNO3_g_aq=kmt_molec(48)
   kHNO3_aq_g=kmt_molec(48)/Hprim(48)
   kH2SO4_g_aq=kmt_molec(49)
   kH2SO4_aq_g=kmt_molec(49)/Hprim(49)
   kMSA_g_aq = kmt_molec(50)
   kMSA_aq_g = kmt_molec(50)/Hprim(50)
