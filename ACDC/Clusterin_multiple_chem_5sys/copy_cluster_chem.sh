...
# perl_directory="./Perl_input"

input_directory=("./Perl_input/SA-NH3/"  "./Perl_input/SA-DMA/"  "./Perl_input/II-IO/"  "./Perl_input/SA-MSA-DMA/")

acdc_perl_script="acdc_2022_11_24.pl"
...
names_to_modify=("acdc_system" "feval" "jeval" "acdc_simulation_setup" "get_cs_aero" "sources_and_constants" "get_scav_parameters" "cluster_dynamics" "clusterin" 
                "get_system_size" "get_bound" "get_charging_state" "get_diameter" "get_diameter_out" "get_mass" "get_nmols_nocharge" 
                "molecule_names_nocharge" "molecule_names" "monomer_indices" "monomer_names" "get_nmols_nocharge_out" "driver_acdc_J" )
...
# names_to_exclude=("dvode" "get_cs_aero" "sources_and_constants")
...
Usage:
./copy_cluster_chem.sh -n number [-dir name] [-chemdir name] [-add name,name,...] [-exc name,name,...] [-bases N,D]  [-v] [-h]
...
IFS=', ' read -r -a nchem_arr <<< "$nchem_str"
...
# echo "In else"
    nchem_arr=( $( seq -w ${nchem_arr[0]} ${nchem_arr[1]} ) )    
    # echo "$( seq ${nchem_arr[0]} ${nchem_arr[1]} )"
fi

IFS=', ' read -r -a bases_arr <<< "$bases_str"


# echo "$nchem_str" | perl -pe 's/\s*//g' | perl -pe 's/-|,/\n/g' 
# echo "$vapor_str" | perl -pe 's/\s*//g' | perl -pe 's/-|,/\n/g' 
echo  " "
# echo "Here $nchem_str , ${nchem_arr[0]}, ${nchem_arr[1]}"
echo "Here $bases_str, ${bases_arr[*]} "
echo "Here ${bases_arr[0]} ${bases_arr[1]} ${bases_arr[2]} "
...
if [ $l_verbose -eq 1 ]; then
    ...
    for str in "${names_to_modify[@]}"; do
        ...
        echo "Modify : ${names_to_modify[@]}"
        echo "Exclude : ${names_to_exclude[@]}"
        # echo "${names_to_modify[@]}"
        # exit
        # exit
...
for nchem in "${nchem_arr[@]}"; do
    ...
    echo "Here when nchem is $nchem, the vapors are H2SO4-NH3 system"
    vap_val=${bases_arr[$nchem-1]}
    use_this_input=${input_directory[$nchem-1]}
    echo "$vap_val , $use_this_input"
    # exit
    ...
    mkdir -p "$subdir"
    perl_in_subdir="$subdir/Perl_input"
    ...
    for fn in "${files_to_copy[@]}"; do
        ...
        cp "$fn" "$fn_cp"
        echo "$subdir"
        cp  "$acdc_perl_script" "$subdir"
        cp -r "$use_this_input" "$perl_in_subdir"
        ...
        # Modify the Perl call to include the suffix in the Perl output
            ...
            ## base name to replace in the run_perl script
            s1=("base1")
            replace_with="$vap_val"
            
            perl -pi -e  "s/${s1}/${replace_with}/g" "$fn_cp"
            echo "${replace_with}"
            # exit
            # perl -pi -e 's/("A" "N")/("A" \b${vap_val})/g' "$run_perl_file"
