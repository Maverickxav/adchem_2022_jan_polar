module acdc_simulation_setup_5
use acdc_system_5
...
subroutine sources_and_constants_5(neqn,source,isconst,fitted)
    ...
    if (solve_ss) then
        ...
        if (small_set_mode) then
            ...
            call monomer_indices_5(n_monomers)
...
end subroutine sources_and_constants_5
...
subroutine get_system_size_5(neq_syst,nclust_syst,nout_syst,nclust_out,nrange_coag,nrange_out,diameter_max_syst)
...
end subroutine get_system_size_5
...
subroutine get_vapor_indices(names_vapor,n1vapor,nmol_vapor)
    ...
    call monomer_names_5(names_monomers)
    call monomer_indices_5(n_monomers)
    call molecule_names_5(names_mol_types)
...
subroutine get_bound_index(nmol_vapor,molec_vapor,nclust_bound,diff_molec_vapor)
    ...
    if (firstcall) then
        ...
        call get_bound_5(bound_clusters,nmols_bound)
        ...
        call get_charging_state_5(ch_state)
...
subroutine get_scav_parameters_5(names_vapor,temperature,c_coag,cs_per_clust,&
    ...
    if (firstcall) then
        ...
        call get_system_size_5(nclust_syst=nclust_syst)
        ...
        call get_diameter_5(dp_clust)
        call get_mass_5(mp_clust)
        ...
        call molecule_names_nocharge_5(names_nocharge_acdc)
        call get_nmols_nocharge_5(nmols_nocharge_clust_acdc)
    ...
    if (.not. present(temperature)) then
        write(*,*) 'get_scav_parameters_5: Temperature must be given as input at every call in the current implementation'
        stop
    end if
...
end subroutine get_scav_parameters_5
...
subroutine get_jbin_parameters(c_out_clust,c_per_aero,comp_nocharge_per_aero,nmols_nocharge_out,&
    ...
    if (firstcall) then
        ...
        call get_system_size_5(nclust_out=nclust_out)
        ...
        call get_diameter_out_5(dp_clust_out)
        dp_clust_out = dp_clust_out*1.d-9           ! nm -> m
        
        ! Get cluster compositions in terms of summed neutral and charged molecules of each species
        call molecule_names_nocharge_5(names_nocharge_acdc)
        call get_nmols_nocharge_out_5(nmols_nocharge_out_acdc)
        names_nocharge_saved = names_nocharge_acdc
...
end module acdc_simulation_setup_5
