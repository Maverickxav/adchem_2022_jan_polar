module driver
use acdc_system, only : nclust, neqn => neq				! numbers of clusters and equations
use acdc_system, only : nout_neu						! numbers for formation fluxes
use acdc_system, only : nout_neg, nout_pos
use solution_settings
use monomer_settings, only : sources_and_constants		! for determining fitted concentrations

implicit none

contains

subroutine acdc_driver(c,cs_ref,temperature,ipr,t_max,t_iter,solve_ss,use_solver,ipar,ok,j_out)
	...
	real(kind(1.d0)), intent(out) :: j_out(3)			! simulated formation rates (neutral, neg and pos) (1/s/m^3)
	...
	real(kind(1.d0)) :: c_out(3)						! total concentration of outgrown particles at the beginning
	...
	real(kind(1.d0)) :: parameters(4)					! one or more of the following parameters (max. 4 param.):
	...
	parameters = (/temperature, cs_ref, ipr, ipr/)
		...
		do while (t_iter_tot .lt. t_max)
			...
			if (solve_ss) then
				...
				if (t_iter_temp .ge. sstimech) then
					if ((maxval(abs((c(1:nclust)-ci_temp(1:nclust))/max(ci_temp(1:nclust),chtol))) .le. sstol)&
